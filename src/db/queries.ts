...
import { LawyerSummary, LawyerProfile, PracticeAreaInfo, JurisdictionInfo, US_STATES, STATE_NAMES } from '../types/index.js';
import { classifyPracticeAreas } from '../data/practice-area-classifier.js';

const BASE_URL = process.env.LAW_AI_BASE_URL || 'https://law.ai';

function buildProfileUrl(slug: string | null, id: number): string {
...
function normalizeState(input: string): string | null {
  ...
  // Full name?
  const code = US_STATES[trimmed.toLowerCase()];
  return code || null;
}
...
const BAR_STATUS_CANONICAL: Record<string, string> = {
  'active': 'Active',
...
function normalizeBarStatus(input: string): string | null {
...
function cleanPracticeAreas(raw: string[] | null): string[] {
  if (!raw) return [];
  return raw
    .map(pa => pa.trim())
...
export async function searchLawyers(params: {
  ...
  limit?: number;
  offset?: number;
  // Advanced filters (Phase 1 additions)
  ...
  if (
    (params.practice_area && params.practice_area.length > MAX_TEXT_LEN) ||
...
export async function getLawyerProfile(lawyerId: number): Promise<LawyerProfile | null> {
  const pool = getPool();
...
export async function findLawyerByName(name: string, state?: string): Promise<LawyerSummary[]> {
  const pool = getPool();
  const conditions: string[] = ['l.is_active = true'];
  const values: any[] = [];
  let paramIdx = 1;
...
export async function getPracticeAreas(): Promise<PracticeAreaInfo[]> {
  const pool = getPool();
...
export async function getJurisdictions(): Promise<JurisdictionInfo[]> {
  ...
  return result.rows.map(row => ({
    state: STATE_NAMES[row.region] || row.region,
...
export async function matchLawyerToMatter(params: {
  description: string;
  location?: string;
  urgency?: string;
  budget?: string;
  limit?: number;
}): Promise<{ matches: MatchResult[]; classified_areas: string[]; classification_details: { area: string; score: number; keywords: string[] }[] }> {
  // Step 1: Classify the description into practice areas
  const classifications = classifyPracticeAreas(params.description);
  ...
  const scored = results.map(lawyer => {
    ...
    // Boost factors
    if (lawyer.claimed) score += 5; // Verified profile
    if (lawyer.rating && lawyer.rating >= 4) score += 3;
    if (lawyer.review_count && lawyer.review_count > 0) score += 1;
  ...
  const matches: MatchResult[] = scored.slice(0, limit).map(({ lawyer, score, matchedAreas }) => {
    ...
    if (matchedAreas.length > 0) explanationParts.push(`Practices ${matchedAreas.join(', ')}`);
    if (lawyer.claimed) explanationParts.push('Verified profile');
    if (lawyer.rating && lawyer.rating >= 4) explanationParts.push(`${lawyer.rating}/5 rating`);
    if (lawyer.city && lawyer.state) explanationParts.push(`Located in ${lawyer.city}, ${lawyer.state}`);
